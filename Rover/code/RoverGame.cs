//#define ForceRunlevelEditor
//#define LevelEditorEnabled
#define DeveleperModeEnabled
//#define TestGameObjectListEnabled
...
namespace RoverGame
{
    ...
    /// </summary>
    public class RoverGame : Microsoft.Xna.Framework.Game, IFreshGame
    {
...
#if TestGameObjectListEnabled
        public static List<GameObject> debugTestObjects = new List<GameObject>(10);
#endif

#if LevelEditorEnabled
        //level editor Variables
        private bool levelEditor = false;
        private RoverGameTile leftMouseTile = new RoverGameTile((int)RoverGameTile.Types.Blocked);
        private RoverGameTile rightMouseTile = new RoverGameTile((int)RoverGameTile.Types.Invalid);
        private TiledLevel levelBeforeChange = null; //temp variable while complex action is active
        private bool levelChanged = false;
        private string editorFilePath = @"Content\maps\editorOutput.txt";
        private static readonly Point LevelEditorInvalidSelectionTile = new Point(-1, -1);
        private Point levelEditorSelectionStartTile = LevelEditorInvalidSelectionTile;
        private Point levelEditorSelectionEndTile = LevelEditorInvalidSelectionTile;
        private TiledLevel.Layer levelEditorActiveLayer = TiledLevel.Layer.Foreground;
        private TiledLevel.Layer levelEditorVisibleLayer = TiledLevel.Layer.All;
        enum EditorShowLevelBoundsMode
        {
            Off,
            Edges,
            TileGrid,
        ...
        private EditorShowLevelBoundsMode editorShowLevelBoundsMode = EditorShowLevelBoundsMode.Edges;
#endif
#if DeveleperModeEnabled
        private bool ranStressTest = false;
#endif
        ...
        private bool mouseClippedToScreen = true;
        ...
        //Sounds
        private bool pauseMusicOnPause = false;
        ...
        private string menuTitle = "Fresh GeoMan";
        ...
        protected override void Initialize()
        {
            ...
            Window.Title = Assembly.GetExecutingAssembly().GetName().Name + " - " + Assembly.GetExecutingAssembly().GetName().Version + " - LevelEditor=" + levelEditor;
#else
            ...
            levelSystem.SetTileType(new RoverGameTile((int)RoverGameTile.Types.Invalid));
            ...
            objectFactory = new RoverGameGameObjectFactory();
            ...
            camera.SetOrigin(paramaters.GameWidth / 2, paramaters.GameHeight / 2);
            camera.SetFocusPosition(paramaters.GameWidth / 2, paramaters.GameHeight / 2);
            ...
            RoverGameHudSystem hud = new RoverGameHudSystem();
            hud.Setup();
            ...
            if (levelEditor)
            {
                gameState = GameStates.LevelEditor;

                levelSystem.SetupLevelEditor(editorFilePath);
            }
            else
#endif
            {
        ...
        void Window_ClientSizeChanged(object sender, EventArgs e)
        {
            ...
            gameManager.SetMaxActivationRadius(width * 1.3f);
            BaseObject.sSystemRegistry.CameraSystem.SetOrigin(width / 2, height / 2);
            BaseObject.sSystemRegistry.GameObjectFactory.UpdateActivationRadii();
            InitializeMenus();
        }
#endif
        ...
        protected override void LoadContent()
        {
            ...
            backgroundPic = Content.Load<Texture2D>(@"pics\misc\cherry-wood_small");
        ...
        private void InitializeMenus()
        {
            ...
            if (debugMenuEnabled)
            {
                ...
                MenuItem debugStreeTestItem = new MenuItem("Run Stress Test", MenuItem_RunStressTest_1FullScreen);
                ...
                Menu debugMenu = mainMenuTree.CreateMenu(new MenuItem[] { debugInfoItem, debugFixedStepItem, debugCameraBoundItem, debugStreeTestItem });
...
#if DeveleperModeEnabled
            if (debugMenuEnabled)
                mainItems = new MenuItem[] { startGameItem, optionsItem, aboutItem, exitItem, debugMenuItem };
            else
#endif
                mainItems = new MenuItem[] { startGameItem, optionsItem, aboutItem, exitItem };
...
#if DeveleperModeEnabled
        private void MenuItem_RunStressTest_1FullScreen()
        {
            mainMenuTree.SwipeAwayMenu(Menu_ClearCurrentMenu);
            GotoFirstLevel(false);
            gameState = GameStates.LoadingLevel;//just in case

            TiledLevel level = new TiledLevel(null);
            int widthInTiles = 40;
            int heightInTiles = 23;
            level.BuildEmpty(32, widthInTiles, heightInTiles);
            Tile[,] foreground = level.GetTileMap(TiledLevel.Layer.Foreground);
            Tile[,] background = level.GetTileMap(TiledLevel.Layer.Background);

            for (int tileX = 0; tileX < widthInTiles; tileX++)
            {
                for (int tileY = 0; tileY < heightInTiles; tileY++)
                {
                    foreground[tileX, tileY] = new Tile((int)RoverGameTile.Types.RandomGhost);
                    background[tileX, tileY] = new Tile((int)RoverGameTile.Types.Mario_Tile_Sky);
                }
            }

            foreground[widthInTiles / 2, heightInTiles / 2] = new Tile((int)RoverGameTile.Types.PlayerSpawn);

            levelSystem.SetCurrentLevel(level);
            levelSystem.RebuildCurrentLevel();

            BaseObject.sSystemRegistry.CollisionSystem.Initialize(level.GetTiledCollisionWorld(), widthInTiles, heightInTiles);
        ...
        private void LoadScriptVariablesPart2()
        {
            ...
            //load level List
            string levelListPath = @"Content\maps\mapList.txt";
            vars.GetVariable("levelListPath", ref levelListPath, true);
            levelSystem.LoadLevelList(levelListPath.ToString(), typeof(RoverLevel));
            ...
            RoverGameGameObjectFactory factory = (RoverGameGameObjectFactory)BaseObject.sSystemRegistry.GameObjectFactory;
        ...
        private void GotoFirstLevel(bool animated)
        {
            ...
            if (ranStressTest)
            {
                Variable var = settingsFile.variables.GetVariable("levelListPath", "no path");
                levelSystem.LoadLevelList(var.String, typeof(TiledLevel));
                ranStressTest = false;
            }
#endif
        ...
        private void UpdatePlayerInput(MouseState mouseState, KeyboardState keyState)
        {
            ...
            //    moveDir.X += gamePad.ThumbSticks.Left.X;
            //    //negative Y because 0,0 is the top left of the screen
            //    moveDir.Y -= gamePad.ThumbSticks.Left.Y;

            //    lookDir.X += gamePad.ThumbSticks.Right.X;
            //    //negative Y because 0,0 is the top left of the screen
            //    lookDir.Y -= gamePad.ThumbSticks.Right.Y;
            ...
            //        moveDir.X += 1;
            //    firePressed = gamePad.IsButtonDown(Buttons.RightTrigger);
            //    PlayerController.LookDir = FreshArchives.NormalizeDown(lookDir, 1);
            //}
            ...
            PlayerController.MovementDir = FreshArchives.NormalizeDown(moveDir, 1);

            //if(lookDir.X != 0 || lookDir.Y != 0)

            PlayerController.FirePressed = firePressed;
        }
        ...
        private void UpdateAllInput()
        {
            ...
            if (mouseInputEnabled)
            {
                ...
                PlayerController.MouseLook = true;//SHOULDFIX handle this better
            ...
            Keys[] keysToListenFor = { Keys.F2, Keys.Escape, Keys.W, Keys.A, Keys.S, Keys.D, Keys.Left, Keys.Right, Keys.Up, Keys.Down, Keys.Space, Keys.Enter, Keys.OemTilde, Keys.D1, Keys.D2, Keys.D3 };
            ...
            {
                UpdatePlayerInput(oldMouseState, oldKeyState);
            }
        }
            ...
            {
                {
                    ...
                    if (gameState == GameStates.MainLoop || gameState == GameStates.LoadingLevel)
                    {
                        ...
                        {
                            levelSystem.Update(updateSeconds, null);
                        }
                    }
...
#if TestGameObjectListEnabled
                data.Add("");
                for (int objectNo = -1; objectNo < debugTestObjects.Count; objectNo++) 
                {
                    GameObject o;
                    string name;
                    if (objectNo == -1)
                    {
                        o = gameManager.GetPlayer();
                        name = "Player";
                    }
                    else
                    {
                        o = debugTestObjects[objectNo];
                        name = "Object#" + objectNo.ToString("D2") + ";";
                    }
                    if (o != null)
                    {
                        string x = ((int)o.GetPosition().X).ToString("D4");
                        string y = ((int)o.GetPosition().Y).ToString("D4");
                        bool offLevel = o.GetPosition().X < 0 || o.GetPosition().Y < 0 || o.GetPosition().X > BaseObject.sSystemRegistry.LevelSystem.GetLevelWidth() || o.GetPosition().Y > BaseObject.sSystemRegistry.LevelSystem.GetLevelHeight();
                        string offlevelST = "";
                        if (offLevel)
                            offlevelST = " - GONE!!";
                        data.Add(name + ": {" + x + "," + y + "}" + offlevelST);
                    }
                }
#endif
    ...
    class PlayerController
    {
        ...
        public static bool RightPressed { get { return MovementDir.X > 0; } }
        public static bool LeftPressed { get { return MovementDir.X < 0; } }

        public static Vector2 MovementDir = Vector2.Zero;
        public static Vector2 LookDir = Vector2.Zero;
